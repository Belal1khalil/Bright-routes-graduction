...
export default function PaymentSuccess() {
  ...
  const { handlePurchaseCompletion } = useContext(CartContext);
  ...
  useEffect(() => {
    // Only clear cart once to prevent infinite loops
    if (!cartCleared) {
      const attemptClearCart = async () => {
        try {
          await ClearCart();
        } catch (error) {
          console.error("Error clearing cart:", error);
        } finally {
          // Mark cart as cleared regardless of success to prevent retries
          setCartCleared(true);
        }
      };
  ...
  }, [session_id, handlePurchaseCompletion, token, ClearCart, cartCleared]);
